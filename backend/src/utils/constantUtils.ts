export const PASSWORD_RULES = {
  MIN_LENGTH: 6,
};
export const MESSAGES = {
  COMMON: {
    SUCCESS: {
      REGISTERED:
        "Registered successfully. OTP sent to email for verification.",
      PAYMENT_SUCCESSFUL: "Payment successful.",
      OTP_VERIFIED: "OTP verified successfully.",
      OTP_SENT: "A new OTP has been sent to your email.",
      LOGIN: "Login successfull",
      PASSWORD_RESET: "Your password reset successfully, you can Login.",
      LOGOUT: "Logged out Successfully",
      PROFILE_UPDATED: "Your profile is updated successfully",
      PASSWORD_CHANGED: "Password changed successfully",
      REFUND_UPDTAE_SUCCESS: "Update refund status successfully.",
      REFUND_SUCCESS: "Refund process success.",
      RETRY_ORDER_PAY_SUCCESS: "Retry payment order suceess.",
      SIGNUP: "Signup successfully.",
      SIGNIN: "Signin successfully.",
      SEND_NOTIFICATION: "Send notification successfully.",
      BLOCK_UPDATE: "Block status updated successfully.",
      PICKUP_APPROVE: "Pickup approved successfully.",
      PICKUP_CANCEL: "Pickup request canceled successfully.",
      PICKUP_RESCHEDULE: "Pickup rescheduled successfully.",
      ADD_TO_WALLET: "has been credited to your wallet.",
      WALLET_PAYMENT_SUCCESS: "Wallet payment successful.",
      RATING_SUCCESS: "Rating submitted successfully!"
    },
    ERROR: {
      FETCH_PAYMENT_FAIL: "Fetch payments failed.",
      CREATE_PAYMENT_FAIL: "Create payment failed.",
      VERIFY_PAYMENT_FAIL: "Verification payment failed.",
      FETCH_PICKUP_FAIL: "Fetch pickup plans failed.",
      UPDATE_PICKUP_FAIL: "Update pickup plans failed.",
      DELETE_PICKUP_FAIL: "Delete pickup plans failed.",
      INVALID_CREDENTIALS: "Invalid credentials.",
      EMAIL_UID_REQUIRED: "Email and UID are required",
      EMAIL_PASSWORD_REQUIRED: "Email and password are required",
      EMAIL_OTP_REQUIRED: "Email and OTP are required",
      ID_REQUIRED: "Id is required.",
      MISSING_IDS: "Missing Id's are required.",
      MISSING_FIELDS: "All fields are required.",
      UNAUTHORIZED: "Unauthorized access.",
      UNAUTHENTICATED: "You are not authenticated.",
      UNKNOWN_ERROR: "An unknown error occurred. Please try again later.",
      ACCOUNT_NOT_VERIFIED: "Account not verified. Please verify your email.",
      EMAIL_NOT_FOUND: "Email not found.",
      INVALID_EXPIRED_OTP: "Invalid or expired OTP",
      OTP_EXPIRED: "OTP has expired. Request a new OTP.",
      INVALID_OTP: "Invalid OTP. Please check and try again.",
      FAILED_OTP: "Failed to send OTP",
      ALREADY_VERIFIED: "Account is already verified.",
      NO_OTP_FOUND: "No OTP found. Please request a new one.",
      BLOCKED: "Your account is blocked.",
      INVALID_BLOCK: "Invalid block status.",
      JWT_SECRET_MISSING: "JWT token is not configured",
      NO_TOKEN: "Token not found",
      POST_OFFICE_ERROR: "No post offices found for this PIN code.",
      PINCODE_ALLOW_ERROR:
        "Only pincodes from Malappuram district are allowed.",
      INVALID_ROLE: "Invalid user role.",
      OTP_SENT: "Failed to send OTP.",
      RESENT_OTP: "Failed to resend OTP.",
      REFRESH_TOKEN: "No refresh token provided.",
      INVALID_TOKEN: "Invalid or expired refresh token.",
      SIGNUP: "Signup failed.",
      INVALID_REFUND_STATUS: "Invalid refund status.",
      LOCATION_REQUIRED: "Location is required.",
      RATING_CREATE_FAILED: "Can't write user rating.",
      WALLET_NOT_FOUND: "Wallet not found.",
      TRANSACTION_NOT_FOUND: "Transaction not found.",
      PAYMENT_IN_PROGRESS: "A payment is already in progress. Please wait a few minutes before retrying.",
      INVALID_SIGNATURE: "Invalid signature. Payment could not be verified.",
      LOCATION_NOT_FOUND: "Location not found.",
      TALUK_NOT_DETERMINE: "Unable to determine taluk.",
      NOTIFICATION_NOT_FOUND: "Notification not found.",
      WALLET_CREATION_FAILED: "Wallet creation failed.",
      INSUFFICIENT_BAL: "Insufficient wallet balance.",
      ETA_NOT_FOUND: "Could not retrieve ETA.",
      LAT_LANG_NOT_FOUND: "Could not retrieve lat/lng.",

    },
  },
  WASTEPLANT: {
    SUCCESS: {
      FETCH: "Fetch waste plants successfully.",
      CREATED: "Waste plant created successfully.",
      UPDATED: "Waste plant updated successfully.",
      DELETED: "Waste plant deleted successfully.",
      PAYMENT: "Payment success.",
      SUBSCRIPTION_PLAN: "Fetch subscription plan successfully.",
      SUBSCRIPTION_CANCEL: "Send subscription cancel request.",
      DROP_SPOT_CREATE: "Dropspot created successfully.",
      DROP_SPOT_UPDATE: "Dropspot updated successfully.",
      DROP_SPOT_DELETE: "Dropspot deleted successfully.",
      FETCH_DROP_SPOT: "Fetch dropspots successfully.",
      WASTE_MWASURED: "Waste measurement saved.",
      CRETAE_TRUCK: "Truck created successfully.",
      FETCH_TRUCK: "Fetch trucks successfully",
      TRUCK_UPDATE: "Truck updated successfully.",
      TRUCK_DELETE: "Truck deleted successfully.",
      FETCH_TRUCK_REQ: "Fetch avaialable truck requests successfully.",
      ASSIGN_TRUCK_DRIVER: "Assign truck to driver successfully.",
      FETCH_USER: "Fetch users successfully.",
      FETCH_WASTE_REPORT: "Fetch waste reports successfully.",
      FETCH_FILTER_WASTE_REPORT: "Fetch filter waste reports successfully.",
      APPROVE_MODIFY: "Pickup modified successfully.",
      REJECT_MODIFY: "Reject pickup modification."
    },
    ERROR: {
      ID_REQUIRED: "Plant ID is required.",
      DOCUMENT_REQUIRED: "License document is required.",
      PDF_FILES_REQUIRED: "Only PDF files are allowed.",
      NOT_FOUND: "Waste Plant not found.",
      FAILED: "Wasteplant updation failed.",
      PAYMENT: "Payment failed.",
      DROP_SPOT_CREATE: "Dropspot failed to create.",
      ID_MISSING: "Plant ID is missing.",
      ACCOUNT_BLOCK: "Your account has been blocked by the superadmin.",
      DRIVER_LIMIT: "You can't add new driver bcoz your plan driver limit is ",
      FAILED_DRIVER: "Can't create driver.",
      TALUK_NOT_FOUND: "Taluk not found in plant.",
      DROPSPOT_NOT_FOUND: "Dropspots not found.",
      DROPSPOT_UPDATE_FAILED: "Dropspot updation failed.",
      DROPSPOT_INVALID_ADDRESS: "Unable to fetch coordinates for the given address.",
      DROPSPOT_CREATE_FAILED: "Dropspot creation failed.",
      PICKUP_FAILED: "Pickup update failed.",
      PICKUP_NOT_FOUND: "Pickup not found.",
      TRUCK_ID_MISS: "Truck ID is missing for the assigned driver.",
      REQ_FREQ_MISS: "Requested frequency is missing.",
      WASTE_REP_NOT_FOUND: "Waste report not found.",
      VEHICLE_NUM_EXISTS: "Vehicle number already exists.",
      TRUCK_FAIL_CREATE: "Can't create truck.",
      TRUCK_NOT_FOUND: "Truck not found.",
      TRUCK_NOT_AVAILABLE: "Selected truck is not available."
    },
  },
  DRIVER: {
    SUCCESS: {
      FETCH_DRIVER: "Fetch drivers successfully.",
      CREATE_DRIVER: "Driver created successfully.",
      UPDATE_DRIVER: "Driver updated successfully.",
      DELETE_DRIVER: "Driver deleted successfully.",
      FETCH_ASSIGN_TRUCK: "Fetch assigned truck successfully.",
      TRUCK_REQ_SENT: "Truck request sent successfully.",
      MARK_RETURN_TRUCK: "Truck marked as returned.",
      MARK_PICKUP_COMPLETED: "Marked pickup as completed.",
      MARK_ATTENDANCE: "Attendance marked successfully",
    },
    ERROR: {
      IMG_ERRROR: "Both license front and back images are required.",
      LAT_LONG_REQUIRED: "Latitude and longitude are required.",
      STATUS_REQUIRED: "TrackingStatus is required.",
      MARK_PICKUP_COMPLETED: "Failed to mark pickup completed.",
      NOT_FOUND: "Driver not found.",
      DRIVERS_NOT_FOUND: "Drivers not found.",
      ADDRESS_NOT_FOUND: "Updated address not found.",
      NOT_MARK_PICKUP: "Not mark pickup.",
      ATTENDANCE_NOT: "Driver attendance not found.",
      ALREADY_SETTLED: "Already settled.",
      UPDATE_FAILED: "Can't update driver profile."
    },
  },
  USER: {
    SUCCESS: {
      RESIDENTIAL_PICKUP: "Fetch residential page successfully.",
      COMMERCIAL_PICKUP: "Fetch commercial page successfully.",
      PICKUP_CANCEL: "Pickup canceled successfully.",
      PICKUP_UPDATED: "Pickup updated successfully.",
      PICKUP_CREATED: "Pickup created successfully.",
      FETCH_DROPSPOTS: "Fetch dropspots successfully.",
      PROFILE_UPDATE: "Profile updated successfully.",
      PICKUP_MODIFY_REQ: "Request submitted successfully"   
    },
    ERROR: {
      NOT_FOUND: "User not found.",
      WASTEPLANT_BLOCK: "Services unavailable: Waste plant is blocked",
      PICKUP_CANCEL: "Failed to cancel pickup.",
      PICKUP_UPDATED: "Failed to update pickup.",
      PICKUP_DATE: "Invalid pickup date format.",
      PICKUP_CREATED: "Failed to create pickup.",
      FETCH_PROFILE: "Failed to fetch Profile.",
      FETCH_EDIT_PROFILE: "Failed to fetch edit profile.",
      PROFILE_UPDATE: "Failed to update profile.",
      PICKUP_MODIFY_REQ: "Failed to submit request.",
      LOC_REQUIRED: "Location is required.",
      PLACE_REQUIRED: "Place is required.",
      RESIDENTIAL_LIMIT: "You can only request one residential pickup per day.",
      MONTHLY_LIMIT: "Monthly residential pickup limit exceeded (max 4).",
      NO_ADDRESS: "No address found.",
      COMPLETE_ADDRESS: "Please complete your address before scheduling a pickup.",
      COMM_MONTHLY_LIMIT: "You already submitted a pickup for this business this month.",
      COMMERCIAL_LIMIT: "You can only submit one commercial pickup request per day.",
      DROPSPOTS_NOT_FOUND: "Drop spots not found.",
      PICKUP_NOT_FOUND: "pickupRequest not found.",
      PAYMENT_NOT_FOUND: "Payment details not found in the pickup request.",
      PAYMENT_COMPLETE: "Payment already completed.",
      PAYMENT_MISMATCH: "Payment amount mismatch.",
      EMAIL_EXIST: "Email already exists. Please use a different email.",
      NAME_EXIST: "Name already exists.",
      LICEENSE_EXIST: "License number already exists.",
      INVALID_PASS: "Invalid email or password.",
      ACCOUNT_BLOCK: "Your account has been blocked by the waste plant.",
      OTP_DATE: "OTP creation date is missing.",
    },
  },

  SUPERADMIN: {
    SUCCESS: {
      DOCUMENT_VERIFIED: "Document has been verified successfully",
      DOCUMENT_REJECTED: "Document has been rejected",
      SUBSCRIPTION_CREATED: "Subscription created successfully",
      SUBSCRIPTION_UPDATED: "Subscription updated successfully",
      SUBSCRIPTION_DELETED: "Subscription deleted successfully",
      PAYMENT_HISTORY: "Fetch payment history successfully",
      SUBSCRIPTION_PLANS: "Fetch subscription plans successfully.",
      ADD_WASTEPLANT: "Fetch add wasteplant succesfully.",
    },
    ERROR: {
      NOT_FOUND: "Superadmin not found.",
      FAILED_DOC_VERIFY: "Document verification failed",
      INVALID_SUBSCRIPTION_NAME: "Subscription plan name is required",
      INVALID_SUBSCRIPTION_DURATION:
        "Valid subscription duration is required (Monthly, 6 Months, Yearly).",
      INVALID_SUBSCRIPTION_PRICE:
        "A valid subscription price greater than 0 is required.",
      SUBSCRIPTION_NOT_FOUND: "Subscription plan not found.",
      SUBSCRIPTION_NOT_ACTIVE: "This subscription plan is not active.",
      SUBS_PAYMENT_NOT_FOUND: "Payments not found.",
      PAYMENT_NOT_FOUND: "Payment not found.",
      PLAN_NOT_EXIST: "Plan not exist.",
      PAYMENT_UPDATE_FAILED: "Payment updation failed.",
      NOT_IN_PLANT: "Pickup not belongs in wasteplant.",
      PAYMENT_NOT_COMPLETE: "Payment not completed.",
      REF_NOT_AFTER_PAYOUT: "Refund not allowed after payout settlement.",
      REF_NOT_REQ: "Refund was not requested.",
      REF_STAT_NOT_NULL: "Refund status cannot be null.",
      CURR_STAT_NOT_NULL: "Current status cannot be null.",
      INVALID_STAT_TRANSITION: "Invalid refund status transition.",
      INVALID_REF_STAT: "Invalid refund status.",
      PAYMENT_ALREADY_REF: "Already refunded.",
      PAYMENT_DET_NOT_MATCH: "Payment details do not match.",
      PAYMENT_NOT_CAPTURE: "Payment is not captured and cannot be refunded.",
      UNSUPPORTED_PAYMENT_METHOD: "Unsupported payment method.",
      USERNAME_EXISTS: "Username already exists.",
      FAILED_ADMIN_CREATION: "Admin not created.",
      PAYMENT_HIS_NOT_FOUND: "Payment history not found.",
      PAYMENT_ID_MISS: "Razorpay Payment ID missing.",
      PLAN_NAME_EXISTS: "Plan name already exists.",
      FAILED_PLANT_CREATION: "Failed to create waste plant.",
      DOCUMENT_NOT_FOUND: "License document not found.",
      PLANTS_NOT_FOUND: "Waste plants not found.",
      DRIVER_LIMIT: "Driver limit must be at least 1",
      TRUCK_LIMIT: "Truck limit must be at least 1",
      USER_LIMIT: "User limit must be at least 1",
      PRICE_LIMIT: "Price must be greater than 0",
      TRIAL_LIMIT: "Trial days must be a non-negative whole number",
    },
  },
};

export const STATUS_CODES = {
  SUCCESS: 200,
  CREATED: 201,
  BAD_REQUEST: 400,
  UNAUTHORIZED: 401,
  FORBIDDEN: 403,
  NOT_FOUND: 404,
  CONFLICT: 409,
  SERVER_ERROR: 500,
  BAD_GATEWAY: 502,
};
