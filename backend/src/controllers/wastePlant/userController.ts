...
import UserService from "../../services/wastePlant/userService";
...
class UserController implements IUserController {
  async fetchUsers(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const { users, total } = await UserService.getAllUsers(wasteplantId, page, limit, search);
  ...
  async userBlockStatus(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const updatedUser = await UserService.userBlockStatusService(
        wasteplantId,
        userId,
        isBlocked
      );
