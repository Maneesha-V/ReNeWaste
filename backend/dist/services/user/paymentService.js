...
let PaymentService = class PaymentService {
    ...
    _walletRepository;
    razorpay;
    constructor(_pickupRepository, _walletRepository) {
    ...
    async createPaymentOrderService(data) {
        const { pickupReqId, userId, amount, method } = data;
        const pickupRequest = await this._pickupRepository.getPickupByUserIdAndPickupReqId(pickupReqId, userId);
    ...
    async verifyPaymentService(data) {
        ...
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, pickupReqId, amount, userId, } = data;
        ...
        let wallet = await this._walletRepository.findWallet(accountId, accountType);
        ...
        await wallet.save();
        return PIckupReqMapper_1.PickupRequestMapper.toPaymentDTO(pickupRequest);
    }
    async getAllPayments(userId, paginationData) {
        const { pickups, total } = await this._pickupRepository.getAllPaymentsByUser(userId, paginationData);
        const payments = pickups.map((p) => PIckupReqMapper_1.PickupRequestMapper.toSummaryDTO(p));
        return { payments, total };
    }
    async rePaymentService(userId, pickupReqId, amount) {
        const pickupRequest = await this._pickupRepository.getPickupByUserIdAndPickupReqId(pickupReqId, userId);
        console.log("pickupRequest ", pickupRequest);
        if (!pickupRequest) {
            throw new Error("pickupRequest not found.");
    ...
    async verifyWalletPickupPayment(userId, paymentData) {
...
exports.PaymentService = PaymentService = __decorate([
    ...
    __param(1, (0, inversify_1.inject)(types_1.default.WalletRepository)),
