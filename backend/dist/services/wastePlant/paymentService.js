...
let PaymentService = class PaymentService {
    ...
    _walletRepository;
    razorpay;
    constructor(pickupRepository, wastePlantRepository, subscriptionPaymentRepository, subscriptionRepository, notificationRepository, superAdminRepository, _walletRepository) {
    ...
    async createPaymentOrder(data) {
        ...
        // }
        const existingPlan = await this.subscriptionRepository.checkPlanNameExist(plant.subscriptionPlan);
        if (!existingPlan || existingPlan.status !== "Active") {
            throw new Error("This subscription plan is not active.");
        }
        const existingInProgressPayment = await this.subscriptionPaymentRepository.findLatestInProgressPayment(plantId);
        const now = new Date();
        if (existingInProgressPayment &&
        ...
        if (existingInProgressPayment &&
            ...
            existingInProgressPayment.inProgressExpiresAt &&
            existingInProgressPayment.inProgressExpiresAt > now) {
            const remainingMinutes = Math.ceil((existingInProgressPayment.inProgressExpiresAt.getTime() -
                now.getTime()) /
                1000 /
                60);
            throw new Error(`A payment is already in progress. Please try again after ${remainingMinutes} minutes.`);
        }
        ...
        const paymentRecord = await this.subscriptionPaymentRepository.createSubscriptionPayment({
            plantId,
            planId,
    ...
    async verifyPaymentService(data) {
        ...
        const updatedPayment = await this.subscriptionPaymentRepository.updateSubscriptionPayment({
        ...
        let adminWallet = await this._walletRepository.findWallet(accountId, accountType);
        if (!adminWallet) {
            try {
                ...
                adminWallet = await this._walletRepository.findWallet(accountId, accountType);
            }
            if (!adminWallet)
    ...
    async fetchSubscriptionPayments(plantId) {
        const plant = await this.wastePlantRepository.getWastePlantById(plantId);
        if (!plant) {
            throw new Error("Plant not found.");
        }
        ...
        const subptnPlanData = await this.subscriptionRepository.checkPlanNameExist(plant?.subscriptionPlan);
        if (!subptnPlanData || !subptnPlanData._id) {
            throw new Error("Subscription plan not exist.");
        }
        const paymentData = await this.subscriptionPaymentRepository.findSubscriptionPayments(plantId);
        if (!paymentData) {
        ...
        return {
            paymentData: SubscriptionPaymentMapper_1.SubscriptionPaymentMapper.mapPopulatedList(paymentData ?? []),
        };
    }
    async retrySubscriptionPayment(data) {
        const { plantId, planId, amount, subPaymtId } = data;
        const subptnPaymentData = await this.subscriptionPaymentRepository.findSubscriptionPaymentById(subPaymtId);
        if (!subptnPaymentData) {
            throw new Error("Payment not found.");
        }
        const now = new Date();
        if (subptnPaymentData.status === "InProgress" &&
        ...
        if (subptnPaymentData.status === "InProgress" &&
            subptnPaymentData.inProgressExpiresAt &&
            subptnPaymentData.inProgressExpiresAt > now) {
            const waitTime = Math.ceil((subptnPaymentData.inProgressExpiresAt.getTime() - now.getTime()) /
                60000);
            throw new Error(`Another payment is in progress. Try again in ${waitTime} minutes.`);
        }
        const newExpiry = new Date(Date.now() + 5 * 60 * 1000);
        const paymentUpdate = {
            amount: amount,
        ...
        const updatedData = await this.subscriptionPaymentRepository.updateSubscriptionPaymentById(subptnPaymentData._id.toString(), paymentUpdate);
    ...
    async updateRefundStatusPayment(data) {
        ...
        if (status === "Processing") {
            pickupReq.payment.inProgressExpiresAt = new Date(Date.now() + 5 * 60 * 1000);
        }
        else {
        ...
        const pickupTransaction = userWallet.transactions.find((tx) => tx.pickupReqId?.toString() === pickupReqId.toString() &&
            tx.subType === "PickupPayment");
        if (!pickupTransaction)
            throw new Error("Pickup transaction not found.");
    ...
    async refundPayment(plantId, data) {
        console.log("data", data);
        const pickupReq = await this.pickupRepository.getPickupById(data.pickupReqId);
        console.log("pickupReq", pickupReq);
        if (pickupReq.wasteplantId?.toString() !== plantId) {
        ...
        try {
            if (payment.method === "Wallet") {
                ...
                const pickupTansaction = wallet.transactions.find((tx) => tx.subType === "PickupPayment" &&
                ...
                const paymentDetails = await this.razorpay.payments.fetch(data.razorpayPaymentId);
                if (paymentDetails.status !== "captured") {
                    throw new Error("Payment is not captured and cannot be refunded.");
                }
                if (process.env.NODE_ENV === "production") {
                    const refund = await this.razorpay.payments.refund(data.razorpayPaymentId, { amount: paymentDetails.amount, speed: "normal" });
            ...
            return PIckupReqMapper_1.PickupRequestMapper.mapPickupReqDTO(pickupReq);
        }
        catch (error) {
...
exports.PaymentService = PaymentService = __decorate([
    ...
    __param(3, (0, inversify_1.inject)(types_1.default.SubscriptionPlanRepository)),
    __param(4, (0, inversify_1.inject)(types_1.default.NotificationRepository)),
    __param(5, (0, inversify_1.inject)(types_1.default.SuperAdminRepository)),
    __param(6, (0, inversify_1.inject)(types_1.default.WalletRepository)),
