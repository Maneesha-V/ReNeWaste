...
exports.PickupRequestMapper = void 0;
class PickupRequestMapper {
    static mapPickupReqDTO(doc) {
        return {
    ...
    static mapGetPickupReqDTO(doc) {
        const primaryAddress = doc.userId?.addresses?.find((addr) => addr._id.toString() === doc.addressId.toString());
        return {
            _id: doc._id.toString(),
            userId: {
    ...
    static toPaymentDTO(doc) {
        return {
            pickupReqId: doc._id.toString(),
            payment: this.mapPayment(doc.payment),
        };
    }
    static toPickupPlansDTO(doc) {
        return {
            _id: doc._id.toString(),
            pickupId: doc.pickupId,
            wasteType: doc.wasteType,
    ...
    static mapToPickupPlansDTO(docs) {
        return docs.map((doc) => this.toPickupPlansDTO(doc));
    }
    static mapPickupReqsDTO(docs) {
        return docs.map((doc) => this.mapPickupReqDTO(doc));
    }
    static mapPickupReqsGetDTO(docs) {
        return docs.map((doc) => this.mapGetPickupReqDTO(doc));
    }
}
exports.PickupRequestMapper = PickupRequestMapper;
