...
const TrackPickup = () => {
  ...
  const { selectedPickup: pickup, loading } = useSelector(
  ...
  useEffect(() => {
    ...
    if(!pickupReqId || !pickup?.userAddress || etaFetched.current) return;
...
console.log({
  loading,
  pickup,
  trackingStatus: pickup?.trackingStatus,
  eta,
});
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-4">
        <Card bordered className="shadow-md w-full h-full p-4">
          <div className="space-y-2 text-sm sm:text-base">
            ...
            <p>
              <strong>Tracking:</strong> {pickup.trackingStatus}
            </p>
            <Button
              type="primary"
